 ...
 */
void cose_encode_protected_header(cose_header *hdr, uint8_t *out, size_t out_size, size_t *out_len)
{
    CborEncoder enc;
    cbor_encoder_init(&enc, out, out_size, 0);
...
/**
 * Decodes cose protected header (CBOR 'bstr' type)
 */
...
/**
 * Decodes cose header (CBOR map) into a struct.
